...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        说明：在encoder-decoder的交互Attention层中len_q(q1,..qt)和len_k(k1,...km)可能不同
        """
...
class MultiHeadAttention(nn.Module):
    """这个Attention类可以实现:
        Encoder的 Self-Attention
        Decoder的 Masked Self-Attention
        Encoder-Decoder的 Attention
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # 然后我们计算 ScaledDotProductAttention 这个函数看一下
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(num_embeddings=src_vocab_size, embedding_dim=d_model)  ## 这个其实就是去定义生成一个矩阵，大小是 src_vocab_size * d_model
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        # torch.gt比较两个矩阵的元素，大于则返回1，否则返回0
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        # 需要将emb维度，通过线性层映射到词表的大小（解码端的词表大小），这样才能通过softmax看当前时刻那个词出现的概率最大
    ...
    def forward(self, enc_inputs, dec_inputs):
        ...
        enc_inputs: [batch_size, src_len]  主要是作为编码段的输入
        dec_inputs: [batch_size, tgt_len]  主要是作为解码端的输入
        """
        # src_len、tgt_len分别为编码端和解码端的句子长度
...
if __name__ == '__main__':
    ...
    enc_inputs, dec_inputs, target_batch = make_data(sentences)
